...
/** Dispara o download de bytes no navegador com o nome de arquivo informado. */
export function saveBytesAsFile(bytes: Uint8Array | Blob, filename: string, mimeType = 'application/pdf') {
...
export async function downloadWatermarkedPdf(
  ...
  saveBytesAsFile(bytes, nomeArquivo);
  return true;
}
